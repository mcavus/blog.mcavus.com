...
theme/templates/     template overrides; files starting with _ are partials
theme/assets/        style.css, replacing the built-in stylesheet
static/              copied verbatim to the site root (icons, images, theme.js)
...
`url` is used for absolute links in the feed and sitemap.

This site overrides the built-in theme, so the `theme` block is not the whole story: `accent`
still sets the light-mode accent, but the page colour and body face come from
`theme/assets/style.css`. Palette and layout there are kept in step with mcavus.com.

`nav` does nothing here. The templates in `theme/templates/` do not include `nav.html`, so entries
under `nav` are ignored.

## Drafts

A post with `draft: true` is work in progress. `overprint serve` renders drafts, because the
preview is for the author. `overprint build` leaves them out of everything — the pages, the index,
the feed, the sitemap, and the tag pages — unless you pass `--drafts`. To publish, set
...
overprint build --drafts      build into dist/ with drafts included
overprint serve --port 4321   build and preview on localhost, drafts included
